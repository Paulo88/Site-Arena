...
document
  ...
  .addEventListener("click", () => mobilePanel.classList.add("open"));
document
  .getElementById("mobileClose")
  .addEventListener("click", () => mobilePanel.classList.remove("open"));
mobilePanel.querySelectorAll("a,button").forEach((el) => {
  el.addEventListener("click", () => {
    if (!el.dataset.open) mobilePanel.classList.remove("open");
  });
});

/* ---------- MODALS ---------- */
/* Inscreva-se / Entrar agora apontam direto para a comunidade do WhatsApp
   (ainda não temos as telas de cadastro/login), então os modais de
   Inscreva-se e Entrar foram removidos. Este bloco de código cobre
   quaisquer outros modais que venham a existir na página (ex: [data-open]
   apontando para um id de modal presente no DOM). */
...
document.querySelectorAll(".modal-overlay").forEach((overlay) => {
  ...
  overlay
    ...
    .forEach((btn) => btn.addEventListener("click", () => closeModal(overlay)));
});
...
function activateTab(tab) {
  ...
  document
    ...
    .forEach((b) => b.classList.toggle("active", b.dataset.tabBtn === tab));
  document
...
function setAudience(value) {
  const btn = document.querySelector(
    '.audience-btn[data-audience="' + value + '"]',
  );
...
function initAnimations() {
  ...
  ScrollTrigger.create({
    ...
    once: true,
    onEnter: () =>
      gsap.to("#xpFill", { width: "62%", duration: 1.6, ease: "power3.out" }),
  });
  ...
  gsap
    ...
    .to(".hero p.lead", { opacity: 1, y: 0, duration: 0.8 }, 0.35)
    .to(".hero-ctas", { opacity: 1, y: 0, duration: 0.8 }, 0.5)
    .to(".stat-hud", { opacity: 1, y: 0, duration: 0.8 }, 0.65);
  ...
  const railSections = Array.from(
    document.querySelectorAll("main [data-rail], .hero[data-rail]"),
  ...
  railSections.forEach((sec, i) => {
    ...
    node.style.top =
      (railSections.length > 1 ? (i / (railSections.length - 1)) * 100 : 0) +
      "%";
  ...
  function setActiveRail(i) {
    ...
    gsap.to(railFill, { height: pct + "%", duration: 0.5, ease: "power2.out" });
  }
...
(function () {
  ...
  introTl
    ...
    .call(() => {
      const s = document.querySelector(".preloader-status");
      if (s) s.textContent = "PRONTO";
    })
    .to(
      ".preloader-content",
